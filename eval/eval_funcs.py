...
def plotPV_roc(rates,names,colours=colours,title="None"):
    ...
    ax[1].grid(True)
    #ax[1].set_yscale("log")
    ax[1].set_xlabel('Real Track Identification True Positive Rate',ha="right",x=1)
    ax[1].set_ylabel('Real Track Identification False Positive Rate',ha="right",y=1)
...
def plot_split_histo(actual,variable,variable_name,range=(0,1),bins=100,title="None"):
    ...
    ax.hist(variable[pv_track_sel],range=range, bins=bins, label="Real tracks", density=True,histtype="stepfilled",color='g',alpha=0.7,linewidth=LINEWIDTH)
    ax.hist(variable[pu_track_sel],range=range, bins=bins, label="Fake tracks", density=True,histtype="stepfilled",color='r',alpha=0.7,linewidth=LINEWIDTH)
